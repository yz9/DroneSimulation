...
class SelectRect: 
    def __init__(self, xmin=0,ymin=0,xmax=1024,ymax=1024, nrects=1, keepcontrol=0,
          optionboxes=0, 
          infoboxes=0, 
          quitLabel="Done",
          imfile=None, 
    ...
    def polyline(self, pointlist, style=0, tags=[] ):
         ...
         elif style==5: self.canvas.create_line(ip,fill='yellow', width=1, tags=tags)
    ...
    def onStart(self, event):
        ...
        self.start = event
        self.drawn = None
        if self.clickhandler: self.clickhandler(event.x, event.y)
